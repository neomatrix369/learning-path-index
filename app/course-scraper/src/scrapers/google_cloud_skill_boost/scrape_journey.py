...
def extract_ml_learning_path(GCSB_JOURNEY_URL) -> list[dict]:
    ...
    for journey in dom.xpath(pages.GCSBLearningJourneyPage.journeys):
        ...
        try:
            ...
            link = urljoin(
                GCSB_HOME_URL, journey.xpath(pages.GCSBLearningJourneyPage.journey_link)
            )
        ...
        data.append(
            {
                'title': (
                    journey.xpath(pages.GCSBLearningJourneyPage.journey_title)[
                        0
                    ].strip()
                ...
                'description': (
                    journey.xpath(pages.GCSBLearningJourneyPage.journey_description)[
                        0
                    ].strip()
...
parser.add_argument(
    '--url',
    required=False,
    default=os.environ.get('GCSB_JOURNEY_URL', CONFIG.GCSB_JOURNEY_URL),
    help='GCSB Journey URL',
)
args = parser.parse_args()


GCSB_JOURNEY_URL = args.url or os.getenv('GCSB_JOURNEY_URL', CONFIG.GCSB_JOURNEY_URL)
