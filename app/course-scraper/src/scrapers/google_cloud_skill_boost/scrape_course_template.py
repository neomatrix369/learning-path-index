...
PARENT_DATA_FOLDER = Path(CONFIG.DATA_PATH)
DATA_FOLDER = Path(CONFIG.DATA_PATH, 'course_data')
DATA_FOLDER.mkdir(exist_ok=True, parents=True)

for learning_path in PARENT_DATA_FOLDER.iterdir():
    if not learning_path.is_file() or ".csv" not in learning_path.suffixes:
        continue
    ...
    for course in csvreader:
        ...
        with open(
            DATA_FOLDER.joinpath(f"{learning_path.name}-Modules-Meta.csv"),
            "a",
            encoding="utf-8",
        ) as f:
    ...
    for course_title, course_module in course_modules_mapping.items():
        ...
        with open(
            DATA_FOLDER.joinpath(f"{course_title}.csv"), "w", encoding="utf-8"
        ) as f:
            excluded_fields = {"steps", "expanded"}
            fieldnames = sorted(
                set(CourseSubmodule.__fields__.keys()) - excluded_fields
            )
            csvwriter = csv.DictWriter(f, fieldnames)
            csvwriter.writeheader()
            csvwriter.writerows(
                [
                    submodule.dict(exclude=excluded_fields)
                    for submodule in parsed_courses
                ]
            )
